...
export default function Thumbnail({ item, showProgress, onAddToList }: ThumbnailProps) {
  ...
  return (
    ...
    >
      <motion.div
        className="relative aspect-video rounded-xl overflow-hidden bg-exyo-surface"
        animate={{
      ...
      >
        ...
        {/* Hover overlay with title on top */}
        <AnimatePresence>
          {isHovered && (
            <motion.div
              initial={{ opacity: 0 }}
            ...
            >
              ...
              <div className="absolute top-0 left-0 right-0 p-3">
                ...
                <div className="flex items-center gap-2 mt-1">
                  ...
                  {year && <span className="text-[11px] text-gray-400">{year}</span>}
                </div>
              </div>
              ...
              <div className="absolute inset-0 flex items-center justify-center">
                <motion.button
                  ...
                  className="w-14 h-14 bg-white/95 hover:bg-white rounded-full flex items-center justify-center shadow-2xl shadow-white/10 transition-all duration-200 hover:scale-110"
                >
              ...
              {onAddToList && isCatalogItem(item) && (
                <motion.button
                  ...
                  className="absolute top-3 right-3 w-8 h-8 bg-black/60 backdrop-blur-sm rounded-full flex items-center justify-center hover:bg-white/20 transition-colors border border-white/10"
                >
