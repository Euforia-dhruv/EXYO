...
export default function ShareButton({ contentId, title, type = 'movie', className = '' }: ShareButtonProps) {
  ...
  return (
    <button
      ...
      disabled={isSharing}
      className={`flex items-center gap-2 bg-white/20 text-white px-4 py-2 rounded font-semibold hover:bg-white/30 transition-colors disabled:opacity-50 ${className}`}
    >
